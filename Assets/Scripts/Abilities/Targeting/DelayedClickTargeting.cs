...
{
    [CreateAssetMenu(fileName = "Delayed Click Targeting", menuName = "Jareds/Abilities/Targeting/Delayed Click", order = 0)]
            ...
            while (!data.IsCancelled())
            {
                ...
                if (Physics.Raycast(PlayerController.GetMouseRay(), out raycastHit, 1000, layerMask))
                {
                    ...
                    if (FindObjectOfType<InputActions>().MovmentControl())
                    {
                        ...
                        yield return new WaitWhile(() => FindObjectOfType<InputActions>().MovmentControl());
